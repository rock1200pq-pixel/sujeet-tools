...
// ════════════════════════════════════════════
// PDF TO WORD (.docx)
// Yeh function script.js ke end mein add karo
// ════════════════════════════════════════════
async function pdfToWord() {
  var file = document.getElementById('pdfToWordFile').files[0];
  if (!file) { setResult('pdfToWordResult', '⚠️ PDF file select karo!', true); return; }

  // docx library load check
  if (typeof docx === 'undefined') {
    setResult('pdfToWordResult', '❌ docx library load nahi hui — page refresh karo!', true);
    return;
  }

  setResult('pdfToWordResult', '⏳ Converting... please wait');

  try {
    var arrayBuffer = await readFileAsArrayBuffer(file);

    // PDF.js se text extract karo
    var pdfDoc = await pdfjsLib.getDocument({ data: new Uint8Array(arrayBuffer) }).promise;
    ...
    for (var i = 1; i <= totalPages; i++) {
      ...
      // Page break — pehle page ke baad
      if (i > 1) {
        paragraphs.push(new docx.Paragraph({ pageBreakBefore: true }));
      }

      // Text items ko lines mein group karo using Y position
      var lines = [];
      var currentLine = '';
      var lastY = null;
      ...
      textContent.items.forEach(function(item) {
        if (!item.str) return;
        var itemY = item.transform[5]; // Y coordinate

        if (lastY === null) {
          // First item
          currentLine = item.str;
          lastY = itemY;
        } else if (Math.abs(itemY - lastY) > 3) {
          // Y changed → new line
          if (currentLine.trim()) {
      ...
      // Lines ko docx paragraphs mein convert karo
      lines.forEach(function(line) {
        paragraphs.push(new docx.Paragraph({ text: line }));
      });
    }

    // Check karo text mila ya nahi
    if (paragraphs.length === 0) {
      setResult('pdfToWordResult', '⚠️ PDF mein koi text nahi mila — yeh image-based PDF ho sakti hai!', true);
      return;
    }
    ...
    var outputName = file.name.replace(/\.pdf$/i, '.docx');
    downloadBlob(blob, outputName);
    setResult('pdfToWordResult', '✅ ' + outputName + ' downloaded! (' + totalPages + ' pages converted)');

  } catch (err) {
    setResult('pdfToWordResult', '❌ Error: ' + err.message, true);
  }
}
